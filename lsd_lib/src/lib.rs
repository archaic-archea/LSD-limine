...
#[global_allocator]
static ALLOCATOR: Alloc = Alloc(vmem::Vmem::new(alloc::borrow::Cow::Borrowed("HEAP"), 1, None));
...
impl Alloc {
    fn handle_alloc_err(&self, layout: vmem::Layout) -> usize {
        let new_space = raw_calls::extend_heap(0x1000);
        self.0.add(new_space as usize, 0x1000);
